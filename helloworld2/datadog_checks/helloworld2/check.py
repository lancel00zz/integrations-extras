...
class Helloworld2Check(AgentCheck):
    def check(self, instance):
        ...
        # Determine log directory (config > env var > platform default)
        DEFAULT_LOG_DIR = '/tmp'

        log_dir = instance.get('log_dir', DEFAULT_LOG_DIR)

        # Now that we know the final path, ensure it exists
        ...
        if not logger.handlers:
            # log_file = '/opt/datadog-agent/logs/helloworld2.log'  # replaced
        ...
        # State tracking
        # state_file = '/opt/datadog-agent/logs/helloworld2_state.json'  # replaced
        log_this_run = False
